...
<Section>
  <SectionTitle title="Technologie" subtitle="které používám" />
  <div class="relative flex py-8 overflow-hidden">
      ...
      >
        {#each featuredTechnologies as technology (technology.name)}
          <div class="relative group">
            ...
            />
            <span
              class="absolute left-1/2 -translate-x-1/2 mt-1 top-full opacity-0 text-sm text-black group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10"
            >
              {technology.name}
            </span>
          </div>
    ...
    {/each}
    <div
      class="absolute left-0 top-0 bottom-0 w-1/5 bg-linear-to-r from-bg-light to-transparent pointer-events-none"
    ></div>
    <div
      class="absolute right-0 top-0 bottom-0 w-1/5 bg-linear-to-l from-bg-light to-transparent pointer-events-none"
    ></div>
  </div>
</Section>
