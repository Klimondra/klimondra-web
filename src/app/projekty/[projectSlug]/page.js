                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between py-6 md:py-8 gap-2 md:gap-8">
                        ...
                        <div className="flex flex-col items-center text-center md:flex-row md:items-center md:text-left gap-3 md:gap-6">
                            <Image
                                ...
                                alt={"Logo projektu " + projectData.visible_name}
                                width={512}
                                height={512}
                        ...
                        <div className="flex-shrink-0 mt-4 md:mt-0">
                            ...
                            >
                                {(projectData.startedDate.getMonth() + 1).toString().padStart(2, '0')}/{projectData.startedDate.getFullYear()}
                                <span className="mx-1.5">-</span>
                                {projectData.endedDate === null ? "Aktivní" : `${(projectData.endedDate.getMonth() + 1).toString().padStart(2, '0')}/${projectData.endedDate.getFullYear()}`}
                            </p>
                        </div>
            ...
            <section className="w-9/10 mx-auto mt-6 sm:mt-10 ">
                <div className={`w-full flex flex-col gap-2.5 sm:gap-3`}>
                    ...
                    <div className={`flex flex-row flex-wrap gap-2`}>
                        {
                            projectData.techStack.map((tech_name, index) => {
                                const tech = techstackList.find((techStack) => techStack.name === tech_name)
                                return (<Link key={index} href={`/projekty?tech=${tech.name}`} className={`flex flex-row gap-2 items-center bg-gradient-to-br from-white/10 to-white/15 backdrop-blur-lg border-1 border-white/10 rounded-lg px-2 py-1 shadow-sm hover:scale-102 active:scale-97 transition-all duration-200`}>
                                    <DynamicIcon lib={tech.icon_lib} name={tech.icon_component} className={`text-2xl lg:text-3xl`} style={{ color: tech.color }} />
                                    <h5 className={`font-readex text-white font-light lg:text-lg`}>{tech.name}</h5>
                                </Link>)
                            })
                        }
                    </div>
            ...
            >
                ...
                <article className={`w-full flex flex-col gap-2.5 sm:gap-3 p-4 sm:p-6 lg:flex-1 xl:flex-1
                    ...
                    <div className={`flex flex-col gap-2`}>
                        {projectData.secondBtn && <Link href={projectData.secondBtn.link} target={"_blank"}
                        ...
                        <Link href={"/projekty"} target={"_blank"}
                              className={`bg-gradient-to-br from-white/10 to-white/15 border-1 border-white/5 text-white font-readex text-base py-2 px-4 rounded-lg
                                    hover:scale-101 active:scale-97 transition-all duration-200`}
                        >
